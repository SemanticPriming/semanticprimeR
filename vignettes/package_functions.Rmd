...
To review the available data from the Linguistic Annotated Bibliography, you can call `import_lab()` with no arguments, which returns the full dataset index describing what is available and which variables/languages each dataset includes.

```{r}
labData <- import_lab()$metadata
head(tibble(labData))
```
